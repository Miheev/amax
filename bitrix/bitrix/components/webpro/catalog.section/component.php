...
//            echo $arSect['ID'].' '.$arSect['NAME'].'<br/>';
        ...
        foreach ($cats as $cat_id=>$cat) {
            $true_filter['SECTION_ID']= $cat;
            if (!$cat_id)
                $true_filter['INCLUDE_SUBSECTIONS']= 'N';
            else
                $true_filter['INCLUDE_SUBSECTIONS']= 'Y';
            $rsElements = CIBlockElement::GetList(array(), $true_filter, false, array('nTopCount'=>4,'nPageSize'=>4), array('ID','NAME'));
            $els= array();
...
//$arSort
    ...
    $arElementLink = array();
    $intKey = 0;
    $sect_offset= (intval($arNavigation["PAGEN"])-1) * intval($arNavigation["SIZEN"]);
    while($arItem = $rsElements->GetNext())
    {
        $arItem['ID'] = (int)$arItem['ID'];
        if (empty($_GET['details'])) {
//        $arItem['sect_id'] = (int)$elem_sect[$sect_offset+$intKey];
            $ss = array_search($arItem['ID'], $real_true_filter);
            $arItem['sect_id'] = $elem_sect[intval($ss)];
        }
    ...
    if (!function_exists('cmp_section')) {
        function cmp_section($a, $b) {
            ...
            $aa= array_search($a['sect_id'], $cats);
            $bb= array_search($b['sect_id'], $cats);
            return ($aa < $bb) ? -1 : 1;
        }
    }
